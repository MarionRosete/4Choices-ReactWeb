    ...
    return (
        ...
        <>
            ...
            <div className='bg-blue-100 rounded-2xl p-5'>
                <p className='text-xl text-blue-800 font-bold'>My Classes </p>
            <center>
            <table className="text-center">
                ...
                <tr className="space-x-1" key={item.id}>
                    ...
                    <td className="flex space-x-1 justify-center">
                        ...
                        <button className= "block bg-blue-900 hover:bg-blue-600 shadow-md  text-white p-0.5 text-sm rounded-md"onClick={()=>{fetch(`http://localhost:8000/api/dashboard/exam/${item.id}`,
                                {headers: { Authorization: `Bearer ${localStorage.getItem('token')}`, 'accept':'application/json'}}
                                ).then(response =>response.json().then(resjson=>{
                                            history.push({pathname:"/myExam", state:{data:resjson.exam, section:item.section, subject:item.subject} })
                                                }
                                                )
                                            )
                                }}>
                        ...
                        <button className= "block bg-red-900 hover:bg-red-600 shadow-md  text-white p-0.5 text-sm rounded-md" onClick={()=>{fetch(`http://localhost:8000/api/dashboard/deleteclass/${item.id}`,
                                {headers: { Authorization: `Bearer ${localStorage.getItem('token')}`, 'accept':'application/json'}}
