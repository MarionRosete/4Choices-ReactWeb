            ...
            <table className="text-center">
                ...
                <tbody>
                {
                    data.map((item)=>
                        <tr key={item.id}>    
                            <td>
                                <button className='text-lg text-blue-600 hover:text-blue-400' 
                                    onClick={()=>{
                                        const url = `http://localhost:8000/api/dashboard/myqa/${item.code}`
                                        fetch(
                                            url,
                                            {
                                                headers: 
                            ...
                            <td>
                                <button className= "block bg-red-900 hover:bg-red-600 shadow-md  text-white p-0.5 text-sm rounded-md" 
                                    onClick={()=>{
                                        const url = `http://localhost:8000/api/dashboard/delete/${item.code}`
                                        fetch(
                                            url,
                                                {
                                                    headers: 
