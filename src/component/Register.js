    ...
    const handleSubmit = (e) => {
            ...
            fetch ("http://localhost:8000/api/register",{
                ...
                body:JSON.stringify(newuser),
                    ...
                    response.json().then(resjson=>{msgSet(resjson); setLoading(false); setMsgErr(resjson.errors)  } 
                    )
        ...
        return (
            ...
            <center > 
                {msg.success===true?
                    ...
                    <>
                    <p>{msg.message}</p>
                    <p>{responsecode.success ===false? "Invalid Code":""}</p>
                ...
                :
                    ...
                    <input className="box-border border focus:outline-none focus:ring-2 focus:ring-blue-500 rounded-sm p-2 my-2 w-80" type="email" placeholder="Email" required value={email} onChange={(e)=>setEmail(e.target.value)} /><br></br>
               
                    <input className="box-border border focus:outline-none focus:ring-2 focus:ring-blue-500 rounded-sm p-2 my-2 w-80" type="password" placeholder="Password" required value={password} onChange={(e)=>setPassword(e.target.value)}/><br></br>
                
                    <input className="box-border border focus:outline-none focus:ring-2 focus:ring-blue-500 rounded-sm p-2 my-2 w-80" type="password" placeholder="Confirm Password" required value={password_confirmation} onChange={(e)=>setPasswordcon(e.target.value)}/><br></br>
                   { !loading ? <button className="transition duration-500 ease-in-out hover:bg-blue-400 transform hover:-translate-y-1 hover:scale-100 rounded-sm bg-blue-500 text-white font-bold font-sans p-2 w-80 ">Sign Up!</button>
                   :
                   <button disabled className="transition duration-500 ease-in-out hover:bg-blue-400 transform hover:-translate-y-1 hover:scale-100 rounded-sm bg-blue-500 text-white font-bold font-sans p-2 w-80 ">Signing Up.....</button>
                    }
